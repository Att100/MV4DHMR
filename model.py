...
class Model(nn.Module):
    ...
    def __init__(self,
        ...
        dict_smpl_layer = {
            'neutral': {
                10: SMPL_Layer(type=smplx_type, gender='neutral', num_betas=10, kid=False, person_center=person_center),
                11: SMPL_Layer(type=smplx_type, gender='neutral', num_betas=11, kid=False, person_center=person_center),
                }
            }
        _moduleDict = []
    ...
    def forward(self,
        ...
        pred_smpl_params, pred_cam, features = self.x_attention_head(z_central, kv, idx_0=idx[0], idx_det=idx, to_smpl=self.smplx_type=='smpl')
        ...
        # Return
        if is_training or not return_list:
            return out, idx
        else:
...
class HPH(nn.Module):
    ...
    def __init__(self,
        ...
        self.decpose_smpl, self.decshape_smpl = nn.Linear(npose, self.joint_rep_dim * 24), nn.Linear(num_betas, num_betas)
    ...
    def forward(self,
        ...
        if to_smpl:
            pred_body_pose = self.decpose_smpl(pred_body_pose)
            pred_betas = self.decshape_smpl(pred_betas)
            num_body_joints = 23
        ...
        # conversion
        pred_body_pose = joint_conversion_fn(pred_body_pose).view(batch_size, num_body_joints+1, 3, 3)
        ...
        return pred_smpl_params, pred_cam, token_out #, pred_smpl_params_list
